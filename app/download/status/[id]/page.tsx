...
export default function DownloadStatusPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    async function validateDownloadId() {
      try {
        const response = await fetch(`/api/youtube/status/${params.id}`);
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <DownloadStatus downloadId={params.id} />
    </div>
  );
} 